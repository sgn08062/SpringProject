...
| 주소 추가 페이지    | /address/insert/{id} | GET    | 폼 화면  |
| 주소 수정 페이지    | /address/update/{id} | GET    | 폼 화면  |

🔌 백엔드 API (RestController, JSON)
| 행동              | API                        | method | 비고                              |
| --------------- | -------------------------- | ------ | ------------------------------- |
| 사용자 주소 목록 전부 조회 | /api/address             | GET    | 응답: JSON 목록(로그인 필요)             |
| 사용자 주소 추가       | /api/address/insert/{id} | POST   | 본문: 주소 정보(JSON)                 |
| 사용자 주소 수정       | /api/address/update/{id} | POST   | 본문: 주소 정보(JSON, addressId 포함) |
| 사용자 주소 삭제       | /api/address/delete/{id} | POST   | 본문: { "addressId": ... }      |
